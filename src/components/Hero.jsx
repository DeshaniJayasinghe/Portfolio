      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-12 lg:gap-24 relative">
        ...
        >
          ...
          <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-xl shadow-gray-200/30">
            ...
            {/* Highlighted Skill Tags with Border */}
            <div className="mt-8 flex flex-wrap gap-4">
              {["Cloud Engineering", "Networking", "Network Security"].map(tag => (
                <span key={tag} className="text-[10px] font-black text-[#5e17eb] border-2 border-[#5e17eb] px-5 py-2 rounded-full uppercase tracking-widest bg-[#5e17eb]/5 shadow-sm">
                  {tag}
                </span>
              ))}
            </div>
        ...
        {/* Right Side: Circle Photo & Straight Card */}
        <motion.div 
        ...
        >
          ...
          {/* Straight Academic Card */}
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ delay: 0.4 }}
            className="w-full max-w-[340px] mt-12 z-20"
          >
